...
KSS    : 1.005 : steady state constant
...
$ODE
double CONC  = 0.5*((CTOT-TARGET-KSS) + sqrt(pow((CTOT-TARGET-KSS), 2.0) + 4.0*KSS*CTOT));        // Free  Drug concentration (central)
...
$TABLE
double CONCFD = 0.5*((CTOT-TARGET-KSS) + sqrt(pow((CTOT-TARGET-KSS), 2.0)+4.0*KSS*CTOT));   // Free Drug concentration (central)
double CONCRC = (TARGET*CONCFD)/(KSS + CONCFD);													                    // Drug-Target Complex concentration (central)
