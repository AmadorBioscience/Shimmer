...
WTVC   : 1.000 : Power law exponent for body weight on V2 (unitless)
...
ETA_V2   : 0.700  0.116  : IIV on V2 (L)
...
double VC  = VCIN * exp(ETA_V2) * pow(WT/WTREF, WTVC);
double R0  = R0IN * exp(ETA_R0);  // baseline total target concentration (nM)
double KD  = KOFF/KON;            // dissociation constant (nM)
...
// Dosing Parameters
tmdd::TMDDR0 = _R(2);   // infusion rate of free drug into central compartment (amount/time
_F(1) = F1;             // bioavailability of free drug in depot (ABS)
_F(2) = 1.0;            


$ODE
double CONC  = 0.5*((CTOT-TARGET-KD) + sqrt(pow((CTOT-TARGET-KD), 2.0) + 4.0*KD*CTOT));           // Free  Drug concentration (central)
dxdt_ABS     = -KA1*ABS;                                                                          // Free  Drug molar amount (depot)
dxdt_CENT    =  KA1*ABS + tmdd::TMDDR0 - (K20 + K23 - KINT)*CONC*VC + K32*PERIPH - KINT*CENT;     // Total Drug molar amount (central)
dxdt_PERIPH  =  K23*CONC*VC - K32*PERIPH;                                                         // Free  Drug molar amount (peripheral)
dxdt_TARGET  =  KSYN - KDEG*TARGET - (KINT - KDEG)*(CTOT - CONC);                                 // Total Target concentration (central)


$TABLE
double CONCFD = 0.5*((CTOT-TARGET-KD) + sqrt(pow((CTOT-TARGET-KD), 2.0)+4.0*KD*CTOT));      // Free Drug concentration (central)
double CONCRC = (TARGET*CONCFD)/(KD + CONCFD);													                    // Drug-Target Complex concentration (central)
